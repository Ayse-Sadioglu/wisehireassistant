...
def create_inconsistent_resumes_with_non_programming_skills():
    ...
    skills = [
        'Git', 'Docker', 'Kubernetes', 'AWS', 'Azure', 'Google Cloud', 'React', 'Angular',
        'Vue.js', 'Node.js', 'HTML', 'CSS', 'SQL', 'NoSQL', 'GraphQL', 'RESTful APIs',
        'CI/CD pipelines', 'Jenkins', 'Ansible', 'Terraform', 'Agile Methodologies', 'Scrum',
        'Project Management', 'JIRA', 'Confluence', 'Adobe Creative Suite', 'Sketch', 'Figma',
        'InVision', 'UX/UI Design Principles', 'Data Analysis', 'Google Analytics', 'SEO'
    ]
    ...
    for i in range(10):
        ...
        doc.add_paragraph(f"{degree}, {university}, Graduated {grad_year}")

        
        doc.add_heading('Job Experience', level=2)
        for _ in range(random.randint(1, 3)):
            company, description = random.choice(companies)
            job_title = random.choice(job_titles)
            start_date = random_date(2008, grad_year)
            end_date = random_date(start_date.year, grad_year)
            ...
            doc.add_paragraph(f"While at {company}, {description}.", style='BodyText')
        ...
        for language in random.sample(languages, random.randint(1, 4)):
            doc.add_paragraph(language, style='ListBullet')
        ...
        filename = f"CV_{i+1}_{name.replace(' ', '_')}.docx"
        doc.save(filename)
        print(f"CV saved: {filename}")

create_inconsistent_resumes_with_non_programming_skills()
