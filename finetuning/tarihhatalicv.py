...
def create_inconsistent_resumes():
    ...
    for i in range(10):
        ...
        job_start_year = random.randint(2008, 2015)
        for _ in range(random.randint(1, 3)):  
            company, work_description = random.choice(companies)
            job_title = random.choice(job_titles)
            job_end_year = job_start_year + random.randint(1, 3)  
            experience = f"{job_title} at {company}, {random_date(job_start_year, job_start_year).strftime('%b %Y')} - {random_date(job_end_year, job_end_year).strftime('%b %Y')}"
            ...
            job_start_year = job_end_year - random.randint(1, 2)
